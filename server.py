...
try:
    ...
    print("Warning: COM3 not alive?, gantry disabled")
...
SQUARE_MM = 47        # mm between square centers
ORIGIN_X  = 10       # mm offset for col 0
ORIGIN_Y  = 10       # mm offset for row 0
FEED      = 2000
# arbitrary ints

def square_to_mm(row, col):
    """Convert board (row, col) to gantry (X, Y) in mm.
    Row 0 = rank 8 (far end), so we flip row to match physical board.
    Col 0 = a-file (left side).
    """
    x_mm = ORIGIN_X + col * SQUARE_MM          # X axis = columns
    y_mm = ORIGIN_Y + (7 - row) * SQUARE_MM    # Y axis = rows, flipped
    return x_mm, y_mm

def gantry_move(start, end):
    if ser is None:
        print("[Gantry] No serial")
        return
    ...
    cmd = f"{sx},{sy},{ex},{ey}\n"
    print(sx, sy, ex, ey, cmd)

    print("[TX]", cmd)
    print("movex =" ,ex-sx)

    ser.write(f"{sx},{sy},{ex},{ey}\n".encode())
